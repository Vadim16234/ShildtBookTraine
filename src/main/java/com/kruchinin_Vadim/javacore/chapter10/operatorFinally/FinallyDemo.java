package com.kruchinin_Vadim.javacore.chapter10.operatorFinally;
...
/*
    В данном примере выполнение метода procA() преждевременно прерывается в блоке оператора try,
    где генерируется исключение, но блок оператора finally все равно выполняется.
    Выход из блока оператора try в методе procB() происходит через оператор return, а блок оператора finally
    выполняется перед возвратом из метода procB(). В методе procC() блок оператора try выполняется обычным образом,
    когда ошибки отсутствуют, но блок оператора finally выполняется все равно.
 */
